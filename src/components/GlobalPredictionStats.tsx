...
const GlobalPredictionStats: React.FC = () => {
  ...
  const [stats, setStats] = React.useState<PredictionAccuracyStats | null>(null);
  ...
  const getTrendIcon = (trend: string) => {
    switch (trend) {
      case 'IMPROVING':
        return <TrendingUpIcon sx={{ color: '#00A532', fontSize: '1rem' }} />;
      case 'DECLINING':
        return <TrendingDownIcon sx={{ color: '#F44336', fontSize: '1rem' }} />;
      default:
        return <TimelineIcon sx={{ color: '#FF9800', fontSize: '1rem' }} />;
    }
  };
